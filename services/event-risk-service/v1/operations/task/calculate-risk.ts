...
import { downStreamError, generalServerFault } from '../../shared/utils/ErrorHandler_ts';

const logger = pino({
  level: process.env.PINO_LOG_LEVEL || 'info',
});

export var calculateRiskTask = async (event: IOracleRequest) => {
  //calls function to convert from oracle to mason common object
  try {
    var reqBody = mapOracleToMason(event);
    logger.debug({ reqBody }, 'Mason Common Object request after mapping');
  } catch (error: any) {
  ...
  try {
    var resBody = await calculateRiskEntity(reqBody);
  } catch (error: any) {
    return downStreamError(error, event, 'Mason-Event-Risk-Service-Entity');
  }

  //Returns back to the original caller with a oracle object
  try {
    const parsedResponse = JSON.parse(JSON.stringify(resBody));
    const response = mapMasonToOracle(parsedResponse);
    logger.debug({ response }, 'Response to client');
    return response;
  } catch (error: any) {
    return generalServerFault(error, event);
  }
 ...
 */
export var mapOracleToMason = (inBody: IOracleRequest): IMasonRequest => {
  /**
  ...
  function dobTransform(inDOB: any): any {
    if (inDOB.length === 10) {
      if (inDOB.substring(2, 3) === '-' && inDOB.substring(5, 6) === '-') {
        return inDOB.substring(6) + '-' + inDOB.substring(0, 2) + '-' + inDOB.substring(3, 5);
      } else {
        // date format is probably correct
        return inDOB;
      }
    } else {
      // not sure the date format
      return '';
    }
  }
  ...
  if (inBody.Parameters) {
    ...
    if (inBody.eventType === 'accountUpdate') {
      ...
      if (inBody.Parameters.previousDob) {
        masonRequest.PreviousCustomer.CustomerInfo!.dob = dobTransform(inBody.Parameters.previousDob);
      }
    ...
    if (inBody.Parameters.dob) {
      masonRequest.Customer!.CustomerInfo!.dob = dobTransform(inBody.Parameters.dob);
    }
    ...
    if (inBody.Parameters.Addresses && inBody.Parameters.Addresses.length > 0) {
      ...
      for (var i = 0; i < inBody.Parameters.Addresses.length; i++) {
        const inAddress = inBody.Parameters.Addresses[i];

        const mappedAddress = buildAddress(inAddress);

        if (
          mappedAddress.HeaderInfo &&
          mappedAddress.HeaderInfo.addressType &&
          mappedAddress.HeaderInfo.addressType === 'billing'
        ) {
    ...
    if (inBody.Parameters.PreviousAddresses && inBody.Parameters.PreviousAddresses.length > 0) {
      ...
      for (var i = 0; i < inBody.Parameters.PreviousAddresses.length; i++) {
        const inPreviousAddress = inBody.Parameters.PreviousAddresses[i];

        const mappedPreviousAddress = buildAddress(inPreviousAddress);

        if (
          mappedPreviousAddress.HeaderInfo &&
          mappedPreviousAddress.HeaderInfo.addressType &&
          mappedPreviousAddress.HeaderInfo.addressType === 'billing'
        ) {
    ...
    if (inBody.Parameters.PreviousPaymentMethods && inBody.Parameters.PreviousPaymentMethods.length > 0) {
      ...
      for (var i = 0; i < inBody.Parameters.PreviousPaymentMethods.length; i++) {
        const inPreviousPayment = inBody.Parameters.PreviousPaymentMethods[i];

        const mappedPreviousPayments = buildPayment(inPreviousPayment);
 ...
 */
export var mapMasonToOracle = (inBody: IMasonResponse): IOracleResponse => {
  ...
  if (inBody.RiskResult) {
    ...
    if (inBody.RiskResult.score) {
      oracleResponse.score = String(inBody.RiskResult.score);
    }
    ...
    if (inBody.RiskResult.DeviceDetail) {
      ...
      if (Object.keys(deviceDetail).length) {
        oracleResponse.DeviceDetail = deviceDetail;
      }
    }
  }
